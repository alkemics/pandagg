...
class Mapping(Tree):
    ...
    def _insert_from_dict(self, name, body, is_subfield, depth, pid=None):
        node = deserialize_field(
            name=name, depth=depth, is_subfield=is_subfield, body=body
        )
    ...
    def _insert_from_node(self, field, depth, is_subfield, pid=None):
        ...
        for subfield in field.fields or []:
            if isinstance(subfield, dict):
                ...
                self._insert_from_dict(
                    name=name,
                    body=body,
                    pid=field.identifier,
                    is_subfield=True,
                    depth=depth + 1,
                )
            elif isinstance(subfield, Field):
                self._insert_from_node(
                    subfield, pid=field.identifier, depth=depth + 1, is_subfield=True
                )
            else:
        ...
        for subfield in field.properties or []:
            if isinstance(subfield, dict):
                ...
                self._insert_from_node(
                    subfield, pid=field.identifier, depth=depth + 1, is_subfield=False
                )
            else:
                raise ValueError("Wrong type %s" % type(field))
    ...
    def _clone(self, identifier, with_tree=False, deep=False):
        return Mapping(identifier=identifier, from_=self if with_tree else None)
    ...
    def list_nesteds_at_field(self, field_path):
        ...
        return [
            ...
            for nid in self.rsearch(
                self[field_path].identifier, filter=lambda n: n.KEY == "nested"
            )
        ]
